...
class LoginViewController: UIViewController {
    ...
    func verificarSesion() {
            if let token = UserDefaults.standard.string(forKey: "authToken") {
                print("Sesión ya iniciada con token: \(token)")
                navigateToHome()
            }
        }
    ...
    func navigateToHome() {
          let storyboard = UIStoryboard(name: "PerfilStoryboard", bundle: nil)
          if let profileVC = storyboard.instantiateViewController(withIdentifier: "PerfilViewController") as? PerfilViewController {
              
              // Establecerlo como la pantalla principal
